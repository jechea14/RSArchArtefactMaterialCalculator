...
def summary(mat_list, amt, experience):
    headers = ["Material", "Amount", "Total Price"]
    name = []
    amount = []
    total_price = []
    ...
    table = zip(name, amount, total_price)
    print(f"\n{tabulate(table, headers=headers)}")

    xpheader = ["Total Cost", "XP with outfit", "XP without outfit", "XP each"]
    table1 = zip(
        ["{:,} gp".format(total_cost(mat_list))],
