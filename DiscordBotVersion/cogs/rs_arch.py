...
class RsArchCalc(commands.Cog):
    ...
    async def calc(self, ctx):
        ...
        async def get_input_of_type(self, func, ctx):
            while True:
                try:
                    msg = await self.bot.wait_for("message", check=check(self, ctx))
        ...
        def webscrape_materials(self, table):
            ...
            return temp_list[1:-5]

        # Webscrape the RS wiki for artifact experience
        def webscrape_experience(self, table):
            experience_data = table.tbody.find_all("tr")[6]
            experience_text = experience_data.find_all("td")[-1].get_text()
            return experience_text.replace(",", "")
        ...
        def calc_mats(self, amount, list):
            for i in range(len(list)):
                ...
                list[i].total_mat_price = amount * list[i].mat_price
        ...
        async with ctx.typing():
            ...
            experience = float(experience_info)
        ...
        embed = discord.Embed(
            title="{:,} {}(s)".format(amt, capitalize), color=discord.Color.blue()
        )
        ...
        for item in mat_list:
            embed.add_field(
                name=item.mat_name,
                value="""Amount: {:,}
                                    Total Price: {:,} gp""".format(
                    item._mat_amount, item._total_mat_price
                ),
            )

        embed.add_field(
            name="Total cost of materials:",
            value="{:,} gp".format(total_cost(self, mat_list)),
            inline=False,
        )

        embed.add_field(
            name="XP with outfit:",
            value="{:,} xp".format(exp_with_outfit(self, amt, artifact_exp)),
            inline=True,
        )

        embed.add_field(
            name="XP without outfit:",
            value="{:,} xp".format(exp_without_outfit(self, amt, artifact_exp)),
            inline=True,
        )
